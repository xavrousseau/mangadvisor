...
def display_recommendation_card(
    ...
    with st.container(border=True):
        ...
        recommendation_goal = recommendation.get("recommendation_goal")
...
tab_single, tab_profile, tab_detail, tab_library = st.tabs(
    [
...
with tab_library:
    ...
    if uploaded_library_file is not None:
        if st.button("Importer le CSV", type="primary", key="library_csv_import_button"):
            uploaded_file_name = uploaded_library_file.name.lower()
    ...
    st.subheader("2. Ajouter ou mettre à jour un manga dans ma bibliothèque")
    ...
    if st.button("Me recommander depuis ma bibliothèque", type="primary"):
        ...
        st.write(f"Objectif utilisé : **{recommendation_goal}**")
